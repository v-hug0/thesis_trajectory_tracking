...
% reference velocities (v_r, w_r), eq. (4.11)
vr = sqrt(dx^2+dy^2);
...
ubar = [vr(k*dt),wr(k*dt)];     % equilibrium (v_r,w_r) at the initial instant
Ur = ubar;
umax = [2.5;2];                 % actuator saturation limits, eq. (4.12)
...
Sx = eye(nz)*4;                 % S_X = 4 I, Results section - circular trajectory
...
syms E1 E2 E4 E5
E = [E1;E2;E4;E5-1];
Z_mon = monomials(E,1:4);

% Error history, used in the phase portrait (Figure 17)
...
for k = 1:round(Tsim/h)  
    %%%%%%% 
    Xr = [Xr xr+h*([vr(k*dt)*cos(xr(3));vr(k*dt)*sin(xr(3));wr(k*dt)])];  % eq. (4.1)
    xr = Xr(:,k+1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%      Plotting the x, y and theta error - Figure 16
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Error = Xr-X;
Error4 = sin(Error(3,:));
Error5 = cos(Error(3,:));
...
%      Figure 17 - Lemma 3.3, Lemma 3.4, Theorem 3.5
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     since S_X = Q^-1 = 4 I at the optimum, condition eq. (3.33) active)
...
% Viewing limits (adjusted to cover the circle's initial condition)
lim = 2.0; 
...
plot3(E1_hist, E2_hist, E3_hist, 'b-', 'LineWidth', 1.5);
...
plot3(0, 0, 0, 'k*', 'MarkerSize', 7, 'LineWidth', 2);
